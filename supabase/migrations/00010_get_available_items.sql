-- Enhanced: Get all items available for a given date range
-- Includes turnaround buffer from app_settings
-- Uses half-open interval logic: [start, end) for reservation
-- Buffer days are added AFTER each reservation ends
  ...
  WHERE i.status = 'active'
    AND NOT EXISTS (
      ...
      WHERE r.item_id = i.id
        ...
        -- Half-open interval with buffer: 
        -- existing reservation blocks [start, end + buffer)
        AND p_start_date < (r.end_date + v_buffer)
